        ...
        <div class="p-0 sm:p-2 md:p-4">
                            ...
                            <div class="px-4 py-5 bg-white sm:p-6">
                                <div class="grid grid-cols-6 gap-6">
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <input wire:model="user.name" type="text" name="name" id="name" class="input-form @error('user.name') is-invalid @enderror" placeholder="{{__('Alias')}}">
                                        @error('user.name') <span class="text-error">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="email" class="label">{{__('E-Mail Address')}}</label>
                                        <input wire:model="user.email" type="email" name="email" id="email" class="input-form @error('user.email') is-invalid @enderror" placeholder="{{__('E-Mail Address')}}">
                                        @error('user.email') <span class="text-error">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="firstname" class="label">{{__('Firstname')}}</label>
                                        <input wire:model="profile.firstname" type="text" name="firstname" id="firstname" class="input-form @error('profile.firstname') is-invalid @enderror" placeholder="{{__('Firstname')}}">
                                        @error('profile.firstname') <span class="text-error">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="lastname" class="label">{{__('Lastname')}}</label>
                                        <input wire:model="profile.lastname" type="text" name="lastname" id="lastname" class="input-form @error('profile.lastname') is-invalid @enderror" placeholder="{{__('Lastname')}}">
                                        @error('profile.lastname') <span class="text-error">{{ $message }}</span> @enderror
                                    </div>
                                    <div class="col-span-6 sm:col-span-3">
                                        <label for="phone" class="label">{{__('Phone')}}</label>
                                        <input wire:model="profile.phone" type="text" name="phone" id="phone" class="input-form @error('profile.phone') is-invalid @enderror" placeholder="{{__('Phone')}}">
            ...
            <form wire:submit.prevent="roles_permissoions">
                <div class="md:grid md:grid-cols-3 md:gap-6" x-show="tab == '#permissions-roles'" x-cloak>
                    <div class="md:col-span-1">
                        <div class="px-4 sm:px-0">
                            <h3 class="text-base sm:text-lg font-medium leading-6 text-gray-900">{{__('Roles')}}</h3>
                            <p class="mt-1 text-sm text-gray-600">
                                {{__('Change the roles for this user')}}.
                            </p>
                        </div>
                    </div>
                    <div class="mt-5 md:mt-0 md:col-span-2">
                    ...
                    <div class="md:col-span-1">
                        <div class="px-4 sm:px-0">
                            <h3 class="text-base sm:text-lg font-medium leading-6 text-gray-900">{{__('Permissions')}}</h3>
                            <p class="mt-1 text-sm text-gray-600">
                                {{__('Change the permissions for this user')}}.
                            </p>
                        </div>
                    </div>
                    <div class="mt-5 md:mt-0 md:col-span-2">
                        <div class="block px-4 pb-2">
                            <div class="inline-block">
                                @foreach ($permissions as $group => $collection)
                                    <div class="py-2">
                                        <span class="text-lg font-semibold">{{__(Str::title($group))}}</span>
                                        <div class="grid sm:grid-cols-2">
