<div>
    <div class="inline-block w-full py 2">
        <button wire:click="toggle_modal" type="button" class="btn-sm btn-primary-border float-right"><i class="fas fa-plus mr-1"></i> {{__('New Tag')}}</button>
    </div>
    @if ($tags->count())
    ...
    <div class="block py-2">
        {{ $tags->links('partials.custom-pagination') }}
    </div>
    @if ($modal)
        @include('livewire.proyect.tags.modal')
    @endif
</div>
