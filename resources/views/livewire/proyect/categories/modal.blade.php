    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <div class="flex justify-between w-full px-4 py-4">
                <h1 class="text-lg font-semibold">{{__('New Category')}}</h1>
                ...
                <div class="bg-white px-4 pb-4">
                    <div class="">
                        <div class="block px-4 pb-2">
                            <label for="name" class="label">{{__('Category Name')}}</label>
                            <input wire:model.lazy="category.name" type="text" name="name" id="name" class="input-form @error('category.name') is-invalid @enderror" placeholder="{{__('Category Name')}}">
                            @error('category.name') <span class="text-error">{{ $message }}</span> @enderror
                        </div>
                        <div class="block px-4 pb-2">
                            <label for="description" class="label">{{__('Description')}}</label>
                            <textarea wire:model.lazy="category.description" name="description" id="description" class="textarea @error('category.description') is-invalid @enderror" placeholder="{{__('Description')}}" rows="3"></textarea>
